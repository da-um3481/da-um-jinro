...
def add_title_slide(prs, title, subtitle=""):
    ...
    fill.solid()
    fill.fore_color.rgb = RGBColor(123, 31, 162)
...
def add_content_slide(prs, title, content_items, emoji=""):
    ...
    for i, item in enumerate(content_items):
        text_box = slide.shapes.add_textbox(Inches(0.8), Inches(content_top + i * 0.7), Inches(8.4), Inches(0.6))
...
def add_program_detail_slide(prs, title, programs, emoji=""):
    ...
    for program in programs:
        card = slide.shapes.add_shape(1, Inches(0.8), Inches(card_top), Inches(8.4), Inches(1.3))
        card.fill.solid()
        card.fill.fore_color.rgb = RGBColor(255, 255, 255)
        card.line.color.rgb = RGBColor(200, 200, 200)
        card.line.width = Pt(1)
        
        title_box = slide.shapes.add_textbox(Inches(1), Inches(card_top + 0.15), Inches(8), Inches(0.4))
        title_frame = title_box.text_frame
        ...
        desc_box = slide.shapes.add_textbox(Inches(1), Inches(card_top + 0.6), Inches(8), Inches(0.6))
...
def add_contact_slide(prs):
    ...
    for i, item in enumerate(contact_items):
        text_box = slide.shapes.add_textbox(Inches(2), Inches(contact_top + i * 0.45), Inches(6), Inches(0.4))
        text_frame = text_box.text_frame
        text_frame.text = item
        para = text_frame.paragraphs[0]
...
def create_presentation():
    ...
    # 1. 표지
    add_title_slide(prs, "경주 근화여자중학교", "1,2학년 겨울방학 자기주도학습 프로그램 제안서")
    
    # 2. 제안 개요
    add_content_slide(prs, "제안 개요", [
    ...
    # 3. 프로그램의 필요성
    add_content_slide(prs, "프로그램의 필요성", [
    ...
    # 4. 프로그램 특징
    add_content_slide(prs, "근화여중 맞춤형 프로그램 특징", [
    ...
    # 5. 1학년 프로그램
    add_program_detail_slide(prs, "1학년 프로그램", [
        {'title': '📖 교과 학습 강화', 'desc': '국어, 영어, 수학, 사회, 과학 등 주요 교과 집중 학습 및 내신 대비'},
        {'title': '✍️ 자기주도학습 훈련', 'desc': '학습 계획 수립, 시간 관리, 효과적인 공부 방법 습득'},
        {'title': '📝 기초 학력 완성', 'desc': '취약 과목 보완 및 학습 기초 다지기'}
    ], "📚")
    
    # 6. 2학년 프로그램
    add_program_detail_slide(prs, "2학년 프로그램", [
        {'title': '📖 교과 심화 학습', 'desc': '주요 교과 심화 학습 및 학습 역량 강화'},
        {'title': '🎯 진로 탐색 및 목표 설정', 'desc': '진로 탐색, 명확한 학습 목표 설정, 동기 부여'},
        {'title': '💪 학습 역량 강화', 'desc': '심화 문제 해결 능력 향상 및 자기주도학습 완성'}
    ], "📚")
    
    # 7. 학습관리 시스템
    add_content_slide(prs, "체계적인 학습관리 시스템", [
        "📅 일일 학습 목표 설정 및 진도 체크",
        "✍️ 학습 일지 작성 및 자기 점검",
        "📊 주간 학습 평가 및 성취도 분석",
        "👩‍🏫 개인별 학습 상담 및 피드백",
    ...
    # 8. 프로그램 운영 계획
    add_content_slide(prs, "프로그램 운영 계획", [
    ...
    # 11. 감사 슬라이드
    add_title_slide(prs, "감사합니다", "근화여자중학교 학생들의 성장을 함께 만들어가겠습니다")
    
    # 저장
    output_file = '/home/user/webapp/근화여자중학교_겨울방학_프로그램_제안서_v2.pptx'
    prs.save(output_file)
    print(f"✅ 수정 제안서 생성 완료: {output_file}")
    return output_file
